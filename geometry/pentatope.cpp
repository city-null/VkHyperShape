...
std::vector<Vertex> generatePentatope() {
    ...
    const float phi = (1.0f + std::sqrt(5.0f)) / 2.0f; // 黄金比例
    const float inv_phi = 1.0f/phi;//0.618
    vertices[0] = Vertex(glm::vec4(1.0f, 0.0f, 0.0f, 0.0f), glm::vec3(1.0f, 0.0f, 0.0f));
    vertices[1] = Vertex(glm::vec4(-inv_phi, phi, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    vertices[2] = Vertex(glm::vec4(-inv_phi, -1.0f, std::sqrt(2.0f + 2.0f/phi), 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    vertices[3] = Vertex(glm::vec4(-inv_phi, -1.0f, -1.0f, std::sqrt(2.0f + 2.0f/phi) * 0.5f), glm::vec3(1.0f, 1.0f, 0.0f));
    vertices[4] = Vertex(glm::vec4(-inv_phi, -1.0f, -1.0f, -std::sqrt(2.0f + 2.0f/phi) * 0.5f), glm::vec3(1.0f, 0.0f, 1.0f));
    return vertices;
}
